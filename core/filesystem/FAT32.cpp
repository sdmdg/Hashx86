...
// --- Directory Internal ---
bool FAT32::FindEntryInCluster(uint32_t cluster, char* name, uint32_t &sectorOut, uint32_t &offsetOut, DirectoryEntryFat32& entryOut) {
    char targetName[8];
    char targetExt[3];
    ...
    while(currentCluster < 0x0FFFFFF8) {
        ...
        for(int s=0; s<bpb.sectorsPerCluster; s++) {
            ...
            for(int i=0; i<16; i++) {
                ...
                bool match = true;
                for(int k=0; k<8; k++) if(dirents[i].name[k] != targetName[k]) match = false;
                for(int k=0; k<3; k++) if(dirents[i].ext[k] != targetExt[k]) match = false;
...
File* FAT32::Open(char* path) {
    ...
    // Copy Name
    int i = 0;
    while(path[i] && i < 127) { file->name[i] = path[i]; i++; }
    file->name[i] = 0;
    ...
    while(currentCluster < 0x0FFFFFF8) {
        ...
        for(int s=0; s<bpb.sectorsPerCluster; s++) {
            ...
            for(int i=0; i<16; i++) {
                ...
                name[8] = 0;
                for(int k=7; k>=0; k--) { if(name[k]==' ') name[k]=0; else break; }
...
void FAT32::CreateFile(char* path) {
    char filename[13];
    uint32_t parentCluster = ParsePath(path, filename);
    if(parentCluster == 0) { printf("Error: Parent directory not found.\n"); return; }
    ...
    uint32_t newCluster = AllocateCluster();
    if(newCluster == 0) { printf("Disk Full\n"); return; }
    
    if(!FindFreeEntryInCluster(parentCluster, s, o)) { printf("Dir Full\n"); return; }
...
void FAT32::DeleteFile(char* path) {
    char filename[13];
    uint32_t parentCluster = ParsePath(path, filename);
    if(parentCluster == 0) { printf("Invalid Path\n"); return; }
...
void FAT32::MakeDirectory(char* path) {
    ...
    memset(dot->name, ' ', 11); dot->name[0] = '.'; dot->attributes = 0x10;
    dot->firstClusterLow = newCluster & 0xFFFF; dot->firstClusterHi = (newCluster>>16)&0xFFFF;
    
    memset(dotdot->name, ' ', 11); dotdot->name[0] = '.'; dotdot->name[1] = '.'; dotdot->attributes = 0x10;
    if(parentCluster == bpb.rootCluster) parentCluster = 0;
    dotdot->firstClusterLow = parentCluster & 0xFFFF; dotdot->firstClusterHi = (parentCluster>>16)&0xFFFF;
...
void FAT32::ReadFile(char* path, uint8_t* buffer, uint32_t length) {
    ...
    while(bytesRead < length && currentCluster < 0x0FFFFFF8) {
        ...
        for(int i=0; i<bpb.sectorsPerCluster; i++) {
            hd->Read28(sector + i, secBuff, 512);
            for(int b=0; b<512; b++) {
                if(bytesRead < length) buffer[bytesRead++] = secBuff[b];
                else break;
            }
...
void FAT32::FormatRaw(AdvancedTechnologyAttachment* hd, uint32_t startSector, uint32_t sizeSectors) {
