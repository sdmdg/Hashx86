...
DEFINE_DRIVER_INFO("BGA Driver for Hashx86",
    "0.1.0",
    {0x1234, 0x1111},  // QEMU / Bochs
...
private:
    ...
    uint32_t FindFramebufferPCI() {
        ...
        // Scan BARs
        for (int i = 0; i < 6; i++) {
            BaseAddressRegister bar = pci.GetBaseAddressRegister(dev->bus, dev->device, dev->function, i);
...
public:
    ...
    void Activate() override {
        ...
        if (this->physFramebufferAddr == 0) {
            printf("\n[BGA] Error: No compatible Graphics Card found via PCI.\n");
            return;
        }

        printf("\n[BGA] Hardware Found. LFB @ 0x%x\n", this->physFramebufferAddr);
