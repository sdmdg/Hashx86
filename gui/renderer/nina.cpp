...
void NINA::DrawBitmapToBuffer(uint32_t* dst, int dstW, int dstH, int dstX, int dstY, uint32_t* src, int srcW, int srcH)
{
...
void NINA::DrawBitmap(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, const uint32_t* bitmapData, int32_t bitmapWidth, int32_t bitmapHeight)
{
        ...
        while (col < bitmapWidth) {
            ...
            while (col < bitmapWidth && (x + col) < bufferWidth) {
                ...
                if (alpha > 0) {
                    ...
                    pixelPtr[col] = (255 << 24) | (blendedRed << 16) | (blendedGreen << 8) | blendedBlue;
                }
                col++;
            }
        }
...
void NINA::FillRectangle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t colorIndex)
{
...
void NINA::DrawRectangle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t colorIndex)
{
    int32_t startX = (x < 0) ? 0 : x;
    int32_t startY = (y < 0) ? 0 : y;
    int32_t endX = (x + w > bufferWidth) ? bufferWidth : x + w;
    ...
    for (int32_t X = startX; X < endX; X++) {
        if (startY >= 0 && startY < bufferHeight)
            buffer[startY * bufferWidth + X] = colorIndex;
    ...
    for (int32_t Y = startY; Y < endY; Y++) {
        if (startX >= 0 && startX < bufferWidth)
            buffer[Y * bufferWidth + startX] = colorIndex;
...
void NINA::FillRoundedRectangle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t radius, uint32_t colorIndex)
{
...
void NINA::DrawRoundedRectangle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, uint32_t w, uint32_t h, uint32_t radius, uint32_t colorIndex)
{
    for (int32_t i = x + radius; i < x + w - radius; ++i) {
        if (i >= 0 && i < bufferWidth) {
            if (y >= 0 && y < bufferHeight)
                buffer[y * bufferWidth + i] = colorIndex;
    ...
    for (int32_t i = y + radius; i < y + h - radius; ++i) {
        if (i >= 0 && i < bufferHeight) {
            if (x >= 0 && x < bufferWidth)
                buffer[i * bufferWidth + x] = colorIndex;
...
void NINA::FillCircle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t cx, int32_t cy, uint32_t radius, uint32_t colorIndex)
{
    int32_t x = radius;
    int32_t y = 0;
    int32_t err = 0;
...
void NINA::DrawCircle(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t cx, int32_t cy, uint32_t radius, uint32_t colorIndex)
{
    int32_t x = radius;
    int32_t y = 0;
    int32_t err = 0;
...
void NINA::DrawHorizontalLine(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, int32_t length, uint32_t colorIndex)
{
    if (y < 0 || y >= bufferHeight) return;
    for (int32_t i = 0; i < length; ++i) {
        int32_t px = x + i;
        if (px >= 0 && px < bufferWidth)
        buffer[y * bufferWidth + px] = colorIndex;
    }
}


void NINA::DrawVerticalLine(uint32_t* buffer, int32_t bufferWidth, int32_t bufferHeight, int32_t x, int32_t y, int32_t length, uint32_t colorIndex)
{
    if (x < 0 || x >= bufferWidth) return;
    for (int32_t i = 0; i < length; ++i) {
        int32_t py = y + i;
        if (py >= 0 && py < bufferHeight)
        buffer[py * bufferWidth + x] = colorIndex;
    }
}
